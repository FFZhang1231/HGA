...
class TGIFQA(Dataset):
    ...
    def __init__(
        ...
        self.resnet_file = os.path.join(self.feat_dir, "TGIF_RESNET_pool5.hdf5")
        self.c3d_file = os.path.join(self.feat_dir, "TGIF_C3D_fc6.hdf5")

        self.resnet_feat = None
        self.c3d_feat = None
    ...
    def get_video_feature(self, key):
        if self.resnet_feat is None:
            self.resnet_feat = h5py.File(self.resnet_file, 'r')
        if self.c3d_feat is None:
            self.c3d_feat = h5py.File(self.c3d_file, 'r')
        video_id = str(key)
        # of shape (frmae_num, 2048)
        resnet_feature = np.array(self.resnet_feat[video_id])
        # of shape (frame_num, 4096)
        c3d_feature = np.array(self.c3d_feat[video_id])
        return resnet_feature, c3d_feature
    ...
    def get_FrameQA_result(self):
        ...
        for index, row in self.data_df.iterrows():
            ...
            question_pad = data_util.question_pad(question, self.q_max_length)
            self.padded_questions.append(question_pad)

            question_length = min(self.q_max_length, len(question))
            self.question_lengths.append(question_length)
    ...
    def getitem_frameqa(self, index):
        key = self.keys[index]
        resnet_feature, c3d_feature = self.get_video_feature(key)
        resnet_pad = data_util.video_pad(resnet_feature,
                                         self.v_max_length).astype(np.float32)
        c3d_pad = data_util.video_pad(c3d_feature,
                                      self.v_max_length).astype(np.float32)
        video_length = min(self.v_max_length, resnet_feature.shape[0])

        return resnet_pad, c3d_pad, video_length, self.padded_questions[
            index], self.question_lengths[index], self.answers[
                index], self.answer_type[index]
    ...
    def get_Count_result(self):
        ...
        for index, row in self.data_df.iterrows():
            # ====== Question ======
            question = self.get_Count_question(index)
            question_pad = data_util.question_pad(question, self.q_max_length)
            self.padded_questions.append(question_pad)
    ...
    def getitem_count(self, index):
        ...
        return resnet_pad, c3d_pad, video_length, self.padded_questions[
            index], self.question_lengths[index], self.answers[index]

    def get_Trans_matrix(self, candidates, is_left=True):
        candidates_matrix = np.zeros([5, self.q_max_length], dtype=np.int64)
        for k in range(5):
    ...
    def get_Trans_result(self):
        ...
        for index, row in self.data_df.iterrows():
            ...
            # candidates = question + candidates, eos=True
            indexed_candidates = [
                self.convert_sentence_to_matrix(raw_question + ' ' + x)
                for x in candidates
            ]

            cand_lens = []
            for cand in indexed_candidates:
                vl = min(self.q_max_length, len(cand))
                cand_lens.append(vl)
            self.candidate_lengths.append(cand_lens)
            # (5, self.q_max_length)
            candidates_pad = self.get_Trans_matrix(indexed_candidates)
            self.padded_candidates.append(candidates_pad)

            # raw candidates, not with question
            indexed_raw_candidates = [
                self.convert_sentence_to_matrix(x) for x in candidates
            ]
            raw_cand_lens = []
            for cand in indexed_raw_candidates:
                vl = min(self.q_max_length, len(cand))
                raw_cand_lens.append(vl)
            self.raw_candidate_lengths.append(raw_cand_lens)
            # (5, self.q_max_length)
            raw_candidates_pad = self.get_Trans_matrix(indexed_raw_candidates)
            self.padded_raw_candidates.append(raw_candidates_pad)
    ...
    def getitem_trans(self, index):
        ...
        self.candidate_lengths = np.asarray(self.candidate_lengths).astype(
            np.int64)
        self.padded_raw_candidates = np.asarray(
            self.padded_raw_candidates).astype(np.int64)
        self.raw_candidate_lengths = np.asarray(
            self.raw_candidate_lengths).astype(np.int64)

        return resnet_pad, c3d_pad, video_length, self.padded_candidates[
            index], self.candidate_lengths[index], self.answers[
                index], self.row_index[index], self.padded_questions[
                    index], self.question_lengths[
                        index], self.padded_raw_candidates[
                            index], self.raw_candidate_lengths[index]