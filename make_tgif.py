...
# VIDEO_FEATURE_DIR = '/data1/jp/Datasets/TGIF-QA'
...
class DatasetTGIF():
    ...
    def read_tgif_from_hdf5(self):
        ...
        if self.image_feature_net.upper() == "CONCAT":
            if self.layer.lower() == "fc":
                ...
                resnet_file = os.path.join(
                    VIDEO_FEATURE_DIR, "TGIF_RESNET_pool5.hdf5")
            elif self.layer.lower() == "conv":
                c3d_file = os.path.join(
                    VIDEO_FEATURE_DIR, "TGIF_C3D_conv5b.hdf5")
                resnet_file = os.path.join(
                    VIDEO_FEATURE_DIR, "TGIF_RESNET_res5c.hdf5")
            return {
                "c3d": h5py.File(c3d_file, 'r'),
                "resnet": h5py.File(resnet_file, 'r')
            }
        else:
            feature_file = os.path.join(
                VIDEO_FEATURE_DIR, "TGIF_" + self.image_feature_net.upper() +
                "_" + self.layer.lower() + ".hdf5")
            assert_exists(feature_file)
            log.info(
                "Load %s hdf5 file: %s",
                self.image_feature_net.upper(), feature_file)
    ...
    def read_df_from_csvfile(self):
        ...
        if self.data_type == 'FrameQA':
            train_data_path = os.path.join(
                self.dataframe_dir, 'Train_frameqa_question.csv')
            test_data_path = os.path.join(
                self.dataframe_dir, 'Test_frameqa_question.csv')
            ...
            self.total_q = pd.read_csv(
                os.path.join(self.dataframe_dir, 'Total_frameqa_question.csv'),
                sep='\t')
        elif self.data_type == 'Count':
            train_data_path = os.path.join(
                self.dataframe_dir, 'Train_count_question.csv')
            test_data_path = os.path.join(
                self.dataframe_dir, 'Test_count_question.csv')
            #self.total_q = pd.DataFrame().from_csv(os.path.join(self.dataframe_dir,'Total_count_question.csv'), sep='\t')
            self.total_q = pd.read_csv(
                os.path.join(self.dataframe_dir, 'Total_count_question.csv'),
                sep='\t')
        elif self.data_type == 'Trans':
            train_data_path = os.path.join(
                self.dataframe_dir, 'Train_transition_question.csv')
            test_data_path = os.path.join(
                self.dataframe_dir, 'Test_transition_question.csv')
            #self.total_q = pd.DataFrame().from_csv(os.path.join(self.dataframe_dir,'Total_transition_question.csv'), sep='\t')
            self.total_q = pd.read_csv(
                os.path.join(
                    self.dataframe_dir, 'Total_transition_question.csv'),
                sep='\t')
        elif self.data_type == 'Action':
            train_data_path = os.path.join(
                self.dataframe_dir, 'Train_action_question.csv')
            test_data_path = os.path.join(
                self.dataframe_dir, 'Test_action_question.csv')
            # self.total_q = pd.DataFrame().from_csv(os.path.join(self.dataframe_dir,'Total_action_question.csv'), sep='\t')
            self.total_q = pd.read_csv(
                os.path.join(self.dataframe_dir, 'Total_action_question.csv'),
                sep='\t')
        ...
        data_df['row_index'] = list(range(
            1, len(data_df) + 1))  # assign csv row index
        return data_df
    ...
    def build_word_vocabulary(
        ...
        pkl.dump(
            ...
            open(
                os.path.join(
                    self.vocabulary_dir,
                    'word_to_index_%s.pkl' % self.data_type), 'w'))
        pkl.dump(
            self.idx2word,
            open(
                os.path.join(
                    self.vocabulary_dir,
                    'index_to_word_%s.pkl' % self.data_type), 'w'))
        ...
        pkl.dump(
            ...
            open(
                os.path.join(
                    self.vocabulary_dir,
                    'ans_to_index_%s.pkl' % self.data_type), 'w'))
        pkl.dump(
            self.idx2ans,
            open(
                os.path.join(
                    self.vocabulary_dir,
                    'index_to_ans_%s.pkl' % self.data_type), 'w'))
        ...
        vocab = pkl.dump(
            ...
            open(
                os.path.join(
                    self.vocabulary_dir,
                    'vocab_embedding_%s.pkl' % self.data_type), 'w'))
    ...
    def get_all_captions(self):
        ...
        qa_data_df = pd.read_csv(
            os.path.join(self.dataframe_dir, TYPE_TO_CSV[self.data_type]),
            sep='\t')
...
# we use resnet C3D
            ...
            if self.layer.lower() == 'fc':
                ...
                c3d_feature = np.transpose(
                    c3d_feature.reshape([-1, 2048, 7, 7]),
                    [0, 2, 3, 1])  # we use resnet C3D
                resnet_feature = np.transpose(
                    resnet_feature.reshape([-1, 2048, 7, 7]), [0, 2, 3, 1])
                video_feature = np.concatenate(
                    (c3d_feature, resnet_feature),
                    axis=len(c3d_feature.shape) - 1)
                #assert list(video_feature.shape[1:]) == [7, 7, 1024+2048]
                assert list(video_feature.shape[1:]) == [
                    7, 7, 2048 + 2048
                ]  # we use resnet C3D
    ...
    def convert_sentence_to_matrix(self, sentence, eos=True):
        ...
        sent2indices = [
            ...
            for w in self.split_sentence_into_words(sentence, eos)
        ]  # 1 is UNK, unknown
        T = len(sent2indices)
    ...
    def get_video_mask(self, video_feature):
        ...
        return data_util.fill_mask(
            self.max_length, video_length, zero_location='LEFT')

    def get_sentence_mask(self, sentence):
        sent_length = len(sentence)
        return data_util.fill_mask(
            self.max_length, sent_length, zero_location='RIGHT')
    ...
    def get_question_mask(self, sentence):
        sent_length = len(sentence)
        return data_util.fill_mask(
            self.max_length, sent_length, zero_location='LEFT')
    ...
    def get_FrameQA_result(self, chunk):
        ...
        batch_question = np.zeros(
            [batch_size, self.max_length], dtype=np.uint32)
        batch_question_right = np.zeros(
            [batch_size, self.max_length], dtype=np.uint32)
        batch_video_mask = np.zeros(
            [batch_size, self.max_length], dtype=np.uint32)
        batch_question_mask = np.zeros(
            [batch_size, self.max_length], dtype=np.uint32)
    ...
    def get_Trans_dict(self, key):
        ...
        candidates_to_indices = [
            self.convert_sentence_to_matrix(question + ' ' + x)
            for x in candidates
        ]
    ...
    def get_Trans_mask(self, candidates):
        ...
        for k in range(5):
            mask_matrix[k] = data_util.fill_mask(
                self.max_length, len(candidates[k]), zero_location='RIGHT')
        return mask_matrix
    ...
    def get_Trans_result(self, chunk):
        ...
        batch_candidates = np.zeros(
            [batch_size, 5, self.max_length], dtype=np.uint32)
        batch_candidates_right = np.zeros(
            [batch_size, 5, self.max_length], dtype=np.uint32)
        batch_answer = np.zeros([batch_size], dtype=np.uint32)

        batch_video_mask = np.zeros(
            [batch_size, self.max_length], dtype=np.uint32)
        batch_candidates_mask = np.zeros(
            [batch_size, 5, self.max_length], dtype=np.uint32)

        batch_debug_sent = np.asarray([None] * batch_size)
        batch_raw_sentences = np.asarray(
            [[None] * 5 for _ in range(batch_size)])  # [batch_size, 5]
        ...
        for k in range(batch_size):
            ...
            answer = int(
                MC_dict['answer']
            )  # a choice from 0-4, since there are five candidate answers
            question = MC_dict['question']
            ...
            candidates_matrix_right = self.get_Trans_matrix(
                candidates, is_left=False)
            ...
            batch_debug_sent[k] = self.data_df.loc[key, 'a' + str(
                int(answer + 1))]
